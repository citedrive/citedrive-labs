...
knex.schema.hasTable('project')
    ...
    knex.schema.hasTable('reference')
        .then((exists) => {
          if (!exists) {
...
*/
  ...
  knex.schema.hasTable('referencePartOfGroup')
      .then((exists) => {
        if (!exists) {
          return knex.schema.createTable('referencePartOfGroup', (table)  => {
            table.string('reference_id').unsigned()
            table.string('group_id').unsigned()
            table.foreign('reference_id').references('reference.guid').onDelete('CASCADE')
            table.foreign('group_id').references('ref_group.guid').onDelete('CASCADE')
      ...
      knex.schema.hasTable('citation')
          .then((exists) => {
            if (!exists) {
              return knex.schema.createTable('citation', (table)  => {
                table.string('guid').primary()
                table.string('prefix')
                table.string('suffix')
        ...
        knex.schema.hasTable('user')
            ...
            knex.schema.hasTable('ref_group')
                .then((exists) => {
                  if (!exists) {
                    return knex.schema.createTable('ref_group', (table)  => {
                      table.string('guid').primary()
                      table.string('name')
                      table.string('type') // folder, ref_list, math_list
                      table.string('parent').unsigned()
                      table.foreign('parent').references('guid').inTable('ref_group').onDelete('CASCADE')
                      table.string('project_id').unsigned() // FOreign Key
                      table.foreign('project_id').references('project.uuid')
                    })
                    .catch((error) => {
